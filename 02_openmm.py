...
fixer = input("Give me absolute path of MINICONDA FOLDER (e.g.: /home/username/miniconda3):"),strip()
line = input('Give me files: ').split()
line = line[0]
goto = "/".join(line.split("/")[:-1])
name = line.split("/")[-1][:-4]
os.chdir(goto)
os.system(f'{fixer}/bin/pdbfixer {name}.pdb --replace-nonstandard --add-residues --output={name}_fixed.pdb')
try:
    reline(name)
